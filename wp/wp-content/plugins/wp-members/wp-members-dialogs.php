...
{ 	
    ...
    $arr = array(__('用户登录', 'wp-members'), __('用户名', 'wp-members'), 'text', 'log', __('密码', 'wp-members'), 'password', 'pwd', 'login', __('登录', 'wp-members'), 'username', 'password');
...
{ 
	$arr = array(__('更改密码', 'wp-members'), __('新密码', 'wp-members'), 'password', 'pass1', __('重复密码', 'wp-members'), 'password', 'pass2', 'pwdchange', __('更新密码', 'wp-members'), 'password', 'password');
...
{ 
	$arr = array(__('Reset Forgotten Password', 'wp-members'), __('用户名', 'wp-members'), 'text', 'user', __('Email', 'wp-members'), 'text', 'email', 'pwdreset', __('Reset Password', 'wp-members'), 'username', 'textbox');
    $str = wpmem_login_form( 'page', $arr );
	return $str;
}
endif;
...
{ 
	...
	if ( $arr[7] == 'login' ) {
		$form = $form . '<tr>
		  <td width="118">&nbsp;</td>
		  <td width="166"><input name="rememberme" type="checkbox" id="rememberme" value="forever" />&nbsp;' . __('记住密码', 'wp-members') . '</td>
		</tr>';
	}
	...
	if ( WPMEM_REGURL != null && $arr[7] == 'login' ) { 
		...
		$form = $form . '<tr>
			<td colspan="2">' . __('新用户?', 'wp-members') . '&nbsp;<a href="'. WPMEM_REGURL . '">' . __('点此注册', 'wp-members') . '</a></td>
		</tr>';
	}
...
function wpmem_inc_registration_OLD( $toggle = 'new', $heading = '' )
{
	...
	if (!$heading) { $heading = "<h2>".__('新用户注册', 'wp-members')."</h2>"; }
	...
	if ($toggle == 'edit') {
		$form = $form . '<tr> 
			  <td width="49%" align="right">' . __('用户名', 'wp-members') . ':</td>
			  <td width="51%" align="left">' . $userdata->user_login . '</td>
			</tr>';
	} else {
		$form = $form . '<tr> 
			  <td width="49%" align="right">' . __('输入用户名', 'wp-members') . '<font color="red">*</font></td>
	...
	$form = $form . '
				...
				<input name="Submit" type="submit" value="' . __( '提交', 'wp-members' ) . '" /> 
				&nbsp;&nbsp; 
				<input name="Reset" type="reset" value="' . __( '重置', 'wp-members' ) . '" />
			  </td>
			</tr>
			<tr>
			  <td>&nbsp;</td>
			  <td><font color="red">*</font> ' . __('必填', 'wp-members') . '</td>
			</tr>';
...
function wpmem_inc_registration_NEW( $toggle = 'new', $heading = ' ')
{
	...
	if( !$heading ) { $heading = __( '新用户注册', 'wp-members' ); }
	...
	if( $toggle == 'edit' ) {
		...
		$form = $form . '<label for="username" class="text">' . __( '输入用户名', 'wp-members' ) . '<font class="req">*</font></label>
			<div class="div_text">
	...
	$form = $form . '<input name="redirect_to" type="hidden" value="' . get_permalink() . '" />
		<div class="button_div">
			<input name="reset" type="reset" value="' . __( '重置', 'wp-members' ) . '" class="buttons" />
			<input name="submit" type="submit" value="' . __( '提交', 'wp-members' ) . '" class="buttons" />
		</div>';
			
	// find a place to put this
	$form = $form . '<font class="req">*</font>' . __( '必填', 'wp-members' ) . '			
...
function wpmem_login_form_NEW( $page, $arr ) 
{
	...
	if ( $arr[7] == 'login' ) {
		$form = $form . '<input name="rememberme" type="checkbox" id="rememberme" value="forever" />&nbsp;' . __('记住密码', 'wp-members') . '&nbsp;&nbsp;';
	}
	...
	if ( ( WPMEM_REGURL != null ) && $arr[7] == 'login' ) { 
		...
		$form = $form . __('新用户?', 'wp-members') . '&nbsp;<a href="' . WPMEM_REGURL . '">' . __('点此注册', 'wp-members') . '</a>';
